...
function PageGoValidateInput(action, sender) {
    ...
    switch (action) {
        case 'page-entry-branch':
            ...
            newDataCollection = BindApiBodyInput('Setup.branch', $('#Id').val().trim() === '0' ? 'INSERT' : 'UPDATE', newDataCollection);
            break;
        case 'page-list-branch':
            newDataCollection = BindApiBodyInput('Setup.branch', 'GETALL', newDataCollection);
            break;
        case 'delete-branch':
            newDataCollection = {
                Id: $(sender).data('id')
            }
            newDataCollection = BindApiBodyInput('Setup.branch', 'DELETE', newDataCollection);
            break;
        case 'edit-branch':
            newDataCollection = {
                Id: $(sender).data('id')
            }
            newDataCollection = BindApiBodyInput('Setup.branch', 'GETBYID', newDataCollection);
            break;
        default:
            Popup.Show('error', 'Invalid action called');
            break;
